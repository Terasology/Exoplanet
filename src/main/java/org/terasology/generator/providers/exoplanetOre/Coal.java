...
@RegisterPlugin
@Produces(ExoplanetOreFacet.Coal.class)
public class Coal extends ExoplanetOreProvider {
    ...
    public void process(GeneratingRegion region) {
        ...
        Border3D border = region.getBorderForFacet(ExoplanetOreFacet.Coal.class);

        ExoplanetOreFacet facet = baseProcess(region, border, INDEX);
        //Create, set the facet for coal
        ExoplanetOreFacet.Coal classFacet = facet.new Coal(region.getRegion(), border);
        classFacet.set(facet.getInternal());
        //Apply
        region.setRegionFacet(ExoplanetOreFacet.Coal.class, classFacet);
    }
}

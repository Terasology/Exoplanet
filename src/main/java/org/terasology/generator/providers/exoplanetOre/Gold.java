...
@RegisterPlugin
@Produces(ExoplanetOreFacet.Gold.class)
public class Gold extends ExoplanetOreProvider {
    ...
    public void process(GeneratingRegion region) {
        ...
        Border3D border = region.getBorderForFacet(ExoplanetOreFacet.Gold.class);

        ExoplanetOreFacet facet = baseProcess(region, border, INDEX);
        //Create, set the facet for gold
        ExoplanetOreFacet.Gold classFacet = facet.new Gold(region.getRegion(), border);
        classFacet.set(facet.getInternal());
        //Apply
        region.setRegionFacet(ExoplanetOreFacet.Gold.class, classFacet);
    }
}

...
@RegisterPlugin
@Produces(ExoplanetOreFacet.Diamond.class)
public class Diamond extends ExoplanetOreProvider {
    ...
    public void process(GeneratingRegion region) {
        ...
        Border3D border = region.getBorderForFacet(ExoplanetOreFacet.Diamond.class);

        ExoplanetOreFacet facet = baseProcess(region, border, INDEX);
        //Create, set the facet for diamond
        ExoplanetOreFacet.Diamond classFacet = facet.new Diamond(region.getRegion(), border);
        classFacet.set(facet.getInternal());
        //Apply
        region.setRegionFacet(ExoplanetOreFacet.Diamond.class, classFacet);
    }
}
